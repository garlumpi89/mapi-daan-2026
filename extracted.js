...
(function () {
  ...
  function getAllGuestNames() {
    ...
    if (!count || isAttendanceNo()) return [];

    const contactName = (contactNameInput && contactNameInput.value || "").trim();
    const names = [];
    if (contactName) names.push(contactName);

    if (count > 1) {
      for (let i = 1; i < count; i++) {
        const input = guestNameFieldsEl ? guestNameFieldsEl.querySelector('[name="guestName' + i + '"]') : null;
  ...
  function renderGuestNameFields() {
    ...
    const count = Number(guestCountSelect.value || 0);
    if (!count || count === 1 || isAttendanceNo()) {
    ...
    const previous = {};
    getGuestNameInputs().forEach(function (input) { previous[input.name] = input.value || ""; });
    const contactName = (contactNameInput && contactNameInput.value || "").trim();

    guestNameFieldsEl.innerHTML = Array.from({ length: count }, function (_, i) {
      const isPrimary = i === 0;
      const current = isPrimary ? contactName : (previous['guestName' + i] || "");
      const label = `${t("rsvp.guest")} ${i + 1}`;
      return `
        <label class="guest-name-card${isPrimary ? ' guest-name-card-primary' : ''}">
          <span>${label}</span>
          <input type="text" class="guest-name-input" name="guestName${i}" value="${escapeHtml(current)}" placeholder="${t("rsvp.guestName.placeholder") || t("rsvp.q3.placeholder")}"${isPrimary ? ' readonly aria-readonly="true" tabindex="-1"' : ' required'}>
        </label>`;
    }).join("");
  ...
  function updateGuestNamesVisibility() {
    ...
    if (!row) return;
    const count = Number(guestCountSelect.value || 0);
    // If only one person is attending, the guest is exactly the contact person,
    // so we hide the names question. For 2+ guests, guest 1 is shown read-only
    // and the remaining guests must be completed individually.
    row.hidden = isAttendanceNo() || count <= 1;
  ...
  document.querySelectorAll(".copy-code").forEach(function (button) {
    button.addEventListener("click", async function () {
      ...
      button.setAttribute("aria-label", t("stay.luze.copied"));
      setTimeout(function () {
  ...
  function validateRsvpBeforeSubmit() {
    ...
    if (isAttending) {
      ...
      renderGuestDetails();
      const detailCards = Array.from(guestDetailsEl ? guestDetailsEl.querySelectorAll(".guest-detail-card") : []);
      if (guestCount && detailCards.length < guestCount) {
        missing.push("guestDetails");
      }
      detailCards.forEach(function (card) {
        const select = card.querySelector(".guest-menu");
